...
- **Backend (Iter34d)** : Module `/app/backend/health_report.py` génère un rapport PDF hebdomadaire via **reportlab 4.5** (1-2 pages A4, charte SAWALI navy/blue). Contenu : KPIs 7 jours (contacts/RDV/interventions/docs/paiements/messages WA-SMS), **3 graphiques de tendance 30 jours** (sparklines avec total/pic via `LinePlot`), état plateforme (jauge support, incidents, planifications), 5 derniers contacts, fiche snapshot joint. Endpoint `GET /api/admin/snapshots/weekly-report-preview` permet aux admins de prévisualiser le PDF on-demand. Le PDF est attaché en complément du `.json.gz` lors de l'envoi auto.
