      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-5 gap-10">
        ...
        <form onSubmit={submit} className="lg:col-span-3 glow-card rounded-2xl p-6 lg:p-8 space-y-4" data-testid="contact-form">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Field label="Nom complet" name="name" value={form.name} onChange={(v) => setForm({ ...form, name: v })} required />
            <Field label="Email" name="email" type="email" value={form.email} onChange={(v) => setForm({ ...form, email: v })} required />
            <Field label="Téléphone" name="phone" value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} />
            <Field label="Entreprise" name="company" value={form.company} onChange={(v) => setForm({ ...form, company: v })} />
          </div>
          <Field label="Sujet" name="subject" value={form.subject} onChange={(v) => setForm({ ...form, subject: v })} />
          <div>
            <label className="block text-xs uppercase tracking-[0.2em] text-slate-400 mb-2">Message *</label>
            <textarea
              required
              rows={6}
              value={form.message}
