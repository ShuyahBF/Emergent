      ...
      <div className="overflow-x-auto ring-1 ring-slate-200 rounded-lg bg-white">
        <table className="min-w-full text-xs" data-testid="err-table">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr><td colSpan="9" className="text-center text-slate-400 py-6">Chargement…</td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan="9" className="text-center text-slate-400 py-6 italic">Aucune erreur.</td></tr>
            ) : items.map((e) => {
              const b = statusBadge(e.StatutEnCours);
              const Icon = b.icon;
              const unack = !e.acknowledged;
              return (
                <tr key={e.id} className={`hover:bg-slate-50 ${unack ? "bg-amber-50/30" : ""}`} data-testid={`err-row-${e.id}`}>
