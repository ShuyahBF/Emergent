    ...
    <div className="space-y-6" data-testid="admin-tracked-page">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Données affichées par client. Sans client défini, aucune donnée n'est affichée à l'utilisateur final.</p>
        </div>
            ...
            <tbody>
              {group.list.map((u) => (
                <tr key={u.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3 text-slate-500 text-xs">{(u.updated_at || u.created_at) ? new Date(u.updated_at || u.created_at).toLocaleString("fr-FR") : "-"}</td>
