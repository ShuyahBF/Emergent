...
- **Backend (Iter34f)** : Nouvel endpoint `GET /api/admin/user-activity` avec filtres `period` (today/week/month/quarter/year/days=N) + `company`. Renvoie `last_logins`, `top_pages`, `totals` (hits/unique_users/unique_companies) + `company_options` pour le dropdown. Lit `db.access_logs` agrégé via Mongo aggregation pipelines.
