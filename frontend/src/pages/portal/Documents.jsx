...
import { FileText, Download, Eye, ImageIcon, Globe } from "lucide-react";
...
export default function ClientDocuments() {
  ...
  const url = (it) => it.file_url ? `${process.env.REACT_APP_BACKEND_URL}${it.file_url}` : null;
  ...
  return (
    <div className="space-y-6" data-testid="client-documents-page">
      <div>
        ...
        <p className="text-sm text-slate-500">Manuels, fiches techniques et annonces qui vous concernent.</p>
      </div>
            ...
            <div key={it.id} className="rounded-xl border border-slate-200 bg-white overflow-hidden" data-testid={`doc-card-${it.id}`}>
              <div className="h-36 bg-gradient-to-br from-sawali-navy to-sawali-navy-dark flex items-center justify-center">
                {it.cover_image_url ? <img src={it.cover_image_url} alt="" className="h-full w-full object-cover" /> :
                  it.file_type === "image" ? <ImageIcon className="h-10 w-10 text-sawali-blue-light/70" /> :
                  it.file_type === "html" || it.body_html ? <Globe className="h-10 w-10 text-sawali-blue-light/70" /> :
              ...
              <div className="p-4">
                ...
                <div className="mt-3 flex items-center gap-3">
                  {url(it) && (
                    <a href={url(it)} target="_blank" rel="noreferrer" className="text-xs inline-flex items-center gap-1 text-sawali-blue hover:underline" data-testid={`doc-download-${it.id}`}>
