    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <p className="mt-4 text-slate-300 max-w-2xl">
          Plongez dans nos missions livrées : contexte, défis, solutions et résultats mesurés.
        </p>
        ...
        {items.length === 0 ? (
          <div className="mt-12 rounded-xl border border-dashed border-white/10 p-16 text-center text-slate-400" data-testid="case-studies-empty">
            ...
            <p className="mt-3">Aucune étude de cas publiée. Revenez prochainement.</p>
          </div>
        ) : (
...
const Card = ({ c }) => (
  <Link to={`/etudes-de-cas/${c.slug}`} className="group glow-card rounded-xl overflow-hidden flex flex-col" data-testid={`case-study-${c.slug}`}>
    <div className="relative h-44 bg-gradient-to-br from-sawali-navy to-sawali-navy-dark overflow-hidden">
      ...
      {c.featured && (
        <span className="absolute top-3 left-3 text-[10px] uppercase tracking-widest bg-sawali-blue px-2 py-1 rounded text-white">Mise en avant</span>
      )}
    </div>
    <div className="p-5 flex-1 flex flex-col">
      ...
      {c.client_name && <p className="text-xs text-slate-400 mt-1">Client : {c.client_name}</p>}
      ...
      {c.tags?.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-1.5">
          {c.tags.slice(0, 4).map((t, i) => (
            <span key={i} className="text-[10px] px-2 py-0.5 rounded bg-sawali-blue/10 text-sawali-blue-light border border-sawali-blue/20">{t}</span>
          ))}
        </div>
      )}

      <span className="mt-5 text-sm text-sawali-blue-light inline-flex items-center gap-1 group-hover:gap-2 transition-all">
        Lire l'étude <ArrowRight className="h-4 w-4" />
      </span>
    </div>
  </Link>
);
